...
export function AdminEntidadesBancariasPage() {
  ...
  const { mutate: editEntidad, isPending: isEditing } = useMutation({
    mutationFn: async ({ id, formData }: { id: number; formData: EntidadBancariaFormType }) => {
      const res = await Api.patch(`/entidad-bancaria/${id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const { mutate: deleteEntidad, isPending: isDeleting } = useMutation({
    mutationFn: async (id: number) => {
      const res = await Api.delete(`/entidad-bancaria/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const handleModalSubmit = (formData: EntidadBancariaFormType) => {
    if (selectedEntidad) {
      editEntidad({ id: selectedEntidad.id, formData });
    } else {
      createEntidad(formData);
    }
  };

  const handleDeleteEntidad = (id: number) => {
