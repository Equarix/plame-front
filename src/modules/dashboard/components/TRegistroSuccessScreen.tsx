...
export function TRegistroSuccessScreen({ data, onRetornar }: TRegistroSuccessScreenProps) {
  const handlePrintCir = () => {
    toast.info("Generación de PDF de constancia de alta (CIR) estará disponible en la siguiente etapa.");
  };

  const fullName = `${data.apellidoPaterno} ${data.apellidoMaterno}, ${data.nombres}`.toUpperCase();
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 bg-[#f8f9fa] dark:bg-zinc-900/40 border border-zinc-200 dark:border-zinc-800/80 rounded-xl overflow-hidden text-xs">
            <div className="p-4 space-y-4 border-b md:border-b-0 md:border-r border-zinc-200 dark:border-zinc-800">
              <div>
                ...
                <span className="text-xs font-bold text-zinc-850 dark:text-zinc-100">L.E / DNI - {data.dni}</span>
              </div>
              <div>
                ...
                <span className="text-xs font-bold text-zinc-850 dark:text-zinc-100">{data.nacionalidad.toUpperCase() || "PERÚ"}</span>
              </div>
            </div>
            ...
            <div className="p-4 space-y-4">
              <div>
                ...
                <span className="text-xs font-bold text-zinc-850 dark:text-zinc-100">{formatDateToDDMMYYYY(data.fechaNacimiento)}</span>
              </div>
              <div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between p-4 bg-white dark:bg-zinc-900 gap-3 text-xs">
                ...
                <div className="w-full sm:w-auto text-left sm:text-right">
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 font-bold underline transition-colors cursor-pointer"
                  >
